/**
 * PDF / Word / PowerPoint / TXT からテキストを抽出して Pinecone に登録
 * 【確定動作版】pdf-parse@2.2.0 + CommonJS + PPTX対応
 */
...
async function extractTextFromPPTX(filePath) {
  console.log(`  📄 PowerPoint処理中: ${path.basename(filePath)}`);
  try {
    const text = await pptxParser(filePath);
    console.log(`    ✓ ${text.length}文字を抽出`);
    return text;
  } catch (e) {
    console.error('    ✗ PowerPointの読み込みエラー:', e.message);
    return null;
  }
}
...
async function main() {
  ...
  console.log('PDF / Word / PowerPoint → Pinecone 登録（確定動作版）');
  console.log('='.repeat(60));
  ...
  const files = fs
    ...
    .filter((f) =>
      ['.pdf', '.doc', '.docx', '.ppt', '.pptx', '.txt'].includes(
